...
function main() {
    ...
    if (firstPlayer === "user") {
        ...
        userDiceIndex = parseInt(getValidUserChoice("Your selection: ", diceInputs.map((_, i) => String(i))));
        aiIndex = getAvailableDiceChoices(diceInputs, userDiceIndex)[0];
        console.log(`I will play with [${diceInputs[aiIndex]}].`);
    } else {
        console.log("I guessed correctly! I will pick first.");
        aiIndex = randomChoice;
        ...
        userDiceIndex = choices[parseInt(getValidUserChoice("Your selection: ", choices.map(String)))];
    }
